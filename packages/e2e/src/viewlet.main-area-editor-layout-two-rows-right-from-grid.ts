...
export const name = 'viewlet.main-area-editor-layout-two-rows-right-from-grid'
export const skip = 1
...
export const test: Test = async ({ Command, FileSystem }) => {
  ...
  assert(savedState.layout.groups.map((group) => group.direction).join(',') === ',2,2', 'Expected right nested rows')
}
