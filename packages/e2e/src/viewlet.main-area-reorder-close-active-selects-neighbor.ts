...
export const name = 'viewlet.main-area-reorder-close-active-selects-neighbor'

export const test: Test = async ({ Command, DragAndDrop, expect, FileSystem, Locator, Main }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const names = ['close-reordered-a.txt', 'close-reordered-b.txt', 'close-reordered-c.txt']
  ...
  await expect(tabTitles[0]).toHaveText('close-reordered-a.txt')
  await expect(tabTitles[1]).toHaveText('close-reordered-b.txt')
  const selectedTab = Locator('.MainTabSelected[title$="close-reordered-a.txt"]')
  await expect(selectedTab).toBeVisible()
  const closedTab = Locator('.MainTab[title$="close-reordered-c.txt"]')
