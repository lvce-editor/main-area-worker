...
export const name = 'viewlet.main-area-editor-layout-alias-two-columns-bottom'
export const skip = 1
...
export const test: Test = async ({ Command, FileSystem }) => {
  ...
  assert(savedState.layout.groups.map((group) => group.direction).join(',') === ',1,1', 'Expected two columns bottom')
}
