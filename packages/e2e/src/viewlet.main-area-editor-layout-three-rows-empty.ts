...
export const name = 'viewlet.main-area-editor-layout-three-rows-empty'
export const skip = 1
...
export const test: Test = async ({ Command, FileSystem }) => {
  ...
  assert(savedState.layout.groups.map((group) => group.size).join(',') === '33.333333,33.333333,33.333334', 'Expected three equal rows')
}
