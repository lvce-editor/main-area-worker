...
import { focusPreviousTab } from '../src/parts/FocusPreviousTab/FocusPreviousTab.ts'

test('focusPreviousTab should select the previous tab in the active group', async () => {
...
test('focusPreviousTab should return state if active group is empty', async () => {
  const state: MainAreaState = {
    ...createDefaultState(),
    layout: {
      activeGroupId: 1,
...
test('focusPreviousTab should return state if active tab is not found', async () => {
  const state: MainAreaState = {
    ...createDefaultState(),
    layout: {
      activeGroupId: 1,
...
test('focusPreviousTab should move from last tab to second-to-last tab', async () => {
  using mockRpc = RendererWorker.registerMockRpc({})
