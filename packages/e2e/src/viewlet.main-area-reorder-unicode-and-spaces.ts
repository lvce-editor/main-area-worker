...
export const name = 'viewlet.main-area-reorder-unicode-and-spaces'

export const test: Test = async ({ Command, DragAndDrop, expect, FileSystem, Locator, Main }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const names = ['alpha file.txt', 'βeta file.txt', '日本語 file.txt']
  ...
  await expect(tabTitles[0]).toHaveText('日本語 file.txt')
  await expect(tabTitles[1]).toHaveText('alpha file.txt')
  await expect(tabTitles[2]).toHaveText('βeta file.txt')
}
