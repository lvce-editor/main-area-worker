...
export const closeAll = async (state: MainAreaState): Promise<MainAreaState> => {
  const { layout } = state
  const editorUids = layout.groups.flatMap((group) => group.tabs.map((tab) => tab.editorUid)).filter((editorUid) => editorUid !== -1)
  await disposeEditors(editorUids)
  const entries = layout.groups.flatMap((group, groupIndex) => {
