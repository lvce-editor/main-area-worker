...
export const name = 'viewlet.main-area-editor-layout-two-columns-bottom-empty'
export const skip = 1
...
export const test: Test = async ({ Command, FileSystem }) => {
  ...
  assert(savedState.layout.groups.map((group) => group.direction).join(',') === ',1,1', 'Expected two bottom columns')
  assert(savedState.layout.groups.map((group) => group.size).join(',') === '50,25,25', 'Expected 50/25/25 sizes')
}
