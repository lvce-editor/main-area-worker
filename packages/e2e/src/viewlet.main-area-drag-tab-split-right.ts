...
export const test: Test = async ({ Command, DragAndDrop, expect, FileSystem, Locator, Main, Workspace }) => {
  ...
  const leftGroupTabs = leftGroup.locator('.MainTab')
  await expect(leftGroupTabs).toHaveCount(1)
  const leftGroupSecondTab = leftGroup.locator('.MainTab[title$="drag-tab-split-right-second.txt"]')
  await expect(leftGroupSecondTab).toBeVisible()
  const rightGroupTabs = rightGroup.locator('.MainTab')
  await expect(rightGroupTabs).toHaveCount(1)
  const rightGroupFirstTab = rightGroup.locator('.MainTab[title$="drag-tab-split-right-first.txt"]')
