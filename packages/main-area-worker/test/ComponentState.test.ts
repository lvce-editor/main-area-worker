...
test('inspects the current virtual DOM without advancing rendered state', () => {
  ...
  const before = MainAreaStates.get(uid)
  ...
  expect(MainAreaStates.get(uid)).toEqual(before)
  expect(MainAreaStates.get(uid).oldState).toBe(oldState)
  expect(MainAreaStates.get(uid).newState).toBe(newState)
})
