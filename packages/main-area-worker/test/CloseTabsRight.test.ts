...
test('closeTabsRight should handle tabs with loading state', () => {
  const state: MainAreaState = {
    ...createDefaultState(),
    layout: {
      activeGroupId: 1,
...
test('closeTabsRight should preserve tab with error message', () => {
  const state: MainAreaState = {
    ...createDefaultState(),
    layout: {
      activeGroupId: 1,
  ...
  expect(result.layout.groups[0].tabs.length).toBe(2)
  expect(result.layout.groups[0].tabs[0].errorMessage).toBe('Failed to load')
  expect(result.layout.groups[0].tabs[0].loadingState).toBe('error')
})
