...
export const renderItems = (oldState: MainAreaState, newState: MainAreaState): any => {
  const { dragOverlay, initial, layout, splitButtonEnabled, tabDropIndicator, uid } = newState
  if (initial) {
    return [ViewletCommand.SetDom2, uid, []]
  }
  const dom = getMainAreaVirtualDom(layout, splitButtonEnabled, dragOverlay, tabDropIndicator)
