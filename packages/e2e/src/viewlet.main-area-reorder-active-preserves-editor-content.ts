...
export const name = 'viewlet.main-area-reorder-active-preserves-editor-content'

export const test: Test = async ({ Command, DragAndDrop, Editor, expect, FileSystem, Locator, Main }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const names = ['content-a.txt', 'content-b.txt', 'content-c.txt']
  ...
  await expect(tabTitles[0]).toHaveText('content-c.txt')
  await expect(tabTitles[1]).toHaveText('content-a.txt')
  await expect(tabTitles[2]).toHaveText('content-b.txt')
