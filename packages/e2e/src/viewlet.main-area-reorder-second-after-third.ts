...
export const name = 'viewlet.main-area-reorder-second-after-third'

export const test: Test = async ({ Command, DragAndDrop, expect, FileSystem, Locator, Main }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const names = ['after-third-a.txt', 'after-third-b.txt', 'after-third-c.txt', 'after-third-d.txt']
  ...
  await expect(tabTitles[0]).toHaveText('after-third-a.txt')
  await expect(tabTitles[1]).toHaveText('after-third-c.txt')
  await expect(tabTitles[2]).toHaveText('after-third-b.txt')
  await expect(tabTitles[3]).toHaveText('after-third-d.txt')
}
