...
export const test: Test = async ({ Command, FileSystem, Main, Workspace }) => {
  ...
  assert(afterLayout.groups[0].id !== afterLayout.groups[1].id, 'Expected distinct groups after split right')
  assert(afterLayout.activeGroupId === afterLayout.groups[1].id, 'Expected new right group to become active')
}
