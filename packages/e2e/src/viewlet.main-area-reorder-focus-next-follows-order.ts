...
export const name = 'viewlet.main-area-reorder-focus-next-follows-order'

export const test: Test = async ({ Command, DragAndDrop, expect, FileSystem, Locator, Main }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const names = ['focus-next-a.txt', 'focus-next-b.txt', 'focus-next-c.txt']
  ...
  await expect(tabTitles[0]).toHaveText('focus-next-c.txt')
  await expect(tabTitles[1]).toHaveText('focus-next-a.txt')
  await expect(tabTitles[2]).toHaveText('focus-next-b.txt')
  const selectedTab = Locator('.MainTabSelected[title$="focus-next-a.txt"]')
