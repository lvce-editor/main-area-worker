...
export const name = 'viewlet.main-area-reorder-modified-tab-becomes-active'

export const test: Test = async ({ Command, DragAndDrop, expect, FileSystem, Locator, Main }) => {
  const tmpDir = await FileSystem.getTmpDir()
  const names = ['modified-inactive-a.txt', 'modified-inactive-b.txt', 'modified-inactive-c.txt']
  const files = names.map((name) => `${tmpDir}/${name}`)
  await FileSystem.setFiles(files.map((uri, index) => ({ content: String(index), uri })))
  await Main.closeAllEditors()
  await Main.openUris(files)
  await Main.handleModifiedStatusChange(files[0], true)
  ...
  await expect(tabTitles[0]).toHaveText('modified-inactive-b.txt')
  await expect(tabTitles[1]).toHaveText('modified-inactive-c.txt')
  await expect(tabTitles[2]).toHaveText('modified-inactive-a.txt')
  const selectedModifiedTab = Locator('.MainTabModified.MainTabSelected[title$="modified-inactive-a.txt"]')
  await expect(selectedModifiedTab).toBeVisible()
}
